...
# Makefile
OSRM_DATA_DIR = osrm
OSRM_PBF = $(OSRM_DATA_DIR)/maluku-latest.osm.pbf
OSRM_FILE = $(OSRM_DATA_DIR)/maluku-latest.osrm

# OSRM: Build map (extract + partition + customize)
osrm-build:
	docker run -t -v $(CURDIR)/$(OSRM_DATA_DIR):/data osrm/osrm-backend osrm-extract -p /opt/car.lua /data/$(notdir $(OSRM_PBF))
	docker run -t -v $(CURDIR)/$(OSRM_DATA_DIR):/data osrm/osrm-backend osrm-partition /data/$(notdir $(OSRM_FILE))
	docker run -t -v $(CURDIR)/$(OSRM_DATA_DIR):/data osrm/osrm-backend osrm-customize /data/$(notdir $(OSRM_FILE))

# OSRM: Run server
# Run OSRM on port 5001
osrm-start:
	docker run -d -p 5001:5000 -v $(CURDIR)/$(OSRM_DATA_DIR):/data osrm/osrm-backend osrm-routed --algorithm mld /data/$(notdir $(OSRM_FILE))

# OSRM: Stop server
osrm-stop:
	docker ps -q --filter ancestor=osrm/osrm-backend | xargs -r docker stop
...
P_9_DIR := 09_project_aggregate_watermark
P_9_PRODUCER_SCRIPT := producer_transactions.py
P_9_CONSUMER_SCRIPT := consumer_transaction.py 
P_9_PACKAGES := org.apache.spark:spark-sql-kafka-0-10_2.12:3.5.0,org.apache.spark:spark-mllib_2.12:3.5.0
